...
public class Button {
    ...
    // コンストラクタ
    public Button(Module module, int x, int y, int width, int height, Minecraft mc) {
        this.module = module;
        this.x = x;
        this.y = y;
        this.width = width;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks, int offset) {
        ...
        mc.fontRenderer.drawStringWithShadow(module.getName(), x + 3, y + offset + (height / 2f - mc.fontRenderer.FONT_HEIGHT / 2f), this.module.istoggled ? 0xFF2B71F3 : -1);
    }
